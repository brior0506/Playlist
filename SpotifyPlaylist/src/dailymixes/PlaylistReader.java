...
public class PlaylistReader {
    ...
    public PlaylistReader(String songsFileName, String playlistsFileName)
        ...
        PlaylistWindow window = new PlaylistWindow(calc);
    ...
    private Playlist[] readPlaylistFile(String playlistFileName)
        ...
        while (file.hasNextLine()
            ...
            if (tokens.length < 8) {
                throw new ParseException("Less than 8 comma "
                    + "seperated values", 0);
            }
            else if (tokens.length > 8) {
                throw new ParseException("More than 8 comma "
                    + "seperated values", 0);
            }
                ...
                if(!this.isInValidPercentRange(minPop, minCountry, minRock) ||
                    !this.isInValidPercentRange(maxPop, maxCountry, maxRock)) {
                    
                    throw new DailyMixDataException("one of the numbers is "
                        + "out of range");
                }
                // 5.TODO: Instantiate a new Playlist with the values
                // populated above
                Playlist newPlaylist = new Playlist(playlistName, minPop, minRock, minCountry,
                    maxPop, maxRock, maxCountry, maxSongsPerPlaylist);
    ...
    private ArrayQueue<Song> readQueueFile(String songFileName)
        ...
        while (file.hasNextLine()) {
            ...
            if(!this.isInValidPercentRange(pop, rock, country)) {
                throw new DailyMixDataException("values not within range");
            }
            ...
            if(tokens.length==5 && !tokens[4].equals("None")) {
                playlist = tokens[4];
            }
